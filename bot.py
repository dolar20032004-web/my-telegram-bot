...
# --- البيانات الأساسية ---
BOT_TOKEN = "8754152241:AAE1dN4fkw2Y8..." # حط توكنك هنا
ADMIN_ID = 123456789  # آيدي الأدمن الأساسي
SMS_MAN_API_KEY = "your_sms_man_api_key_here"  # مفتاح API الخاص بـ SMS-Man

# قاعدة بيانات مؤقتة (تستبدل بقاعدة بيانات حقيقية لاحقاً)
users_db = {}
balances = {}
forced_channels = []  # قائمة قنوات الاشتراك الإجباري
...
async def check_subscription(user_id: int, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    for channel in forced_channels:
        try:
            ...
            # إذا فشل الفحص (مثلاً البوت مو مشرف بالقناة)، نعتبره غير مشترك أو نتجاوزه
            pass
    return True
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        f"أهلاً بك يا {user.first_name} في بوت الخدمات الرقمية وأرقام الوهمية.\n\n"
    ...
    # تنظيف القائمة من الأزرار الفارغة للأعضاء العاديين
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "check_sub":
        ...
        await query.message.edit_text("🛒 خدمة شراء الأرقام قيد التفعيل عبر API الخاص بـ SMS-Man.")

    elif query.data == "admin_panel" and user_id == ADMIN_ID:
        panel_text = "⚙️ **لوحة تحكم الأدمن الرئيسية:**"
        keyboard = [
            [InlineKeyboardButton("➕ إيقاف/تفعيل الصيانة", callback_data="toggle_maint")],
